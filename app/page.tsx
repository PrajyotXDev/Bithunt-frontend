...
import { ArrowDownRight, Terminal, Zap, ChevronDown } from "lucide-react";
...
export default function Home(){
 ...
 <section id="rules" className="section rules"><div className="section-index">03 / RULES</div><div className="section-heading"><p>MISSION PARAMETERS</p><h2>KNOW THE<br/><span>RULES.</span></h2></div><div className="rule-grid">{["Problem-solving is the mission.","Any programming language.","Maximum team size: 4.","Competition duration: 3 hours."].map((x,i)=><div className="rule" key={x}><span>0{i+1}</span><p>{x}</p></div>)}</div></section>
 <section id="faq" className="section faq"><div className="section-index">04 / FAQ</div><div className="faq-list">{faqs.map(([q,a],i)=><button className={"faq-item "+(open===i?"open":"")} key={q} onClick={()=>setOpen(open===i?null:i)}><span>0{i+1}</span><strong>{q}</strong><ChevronDown size={18}/><p>{a}</p></button>)}</div></section>
 <section id="register" className="register"><div className="register-bg">BITHUNT</div><p>FINAL TRANSMISSION // REGISTRATION</p><h2>READY TO<br/><span>HUNT?</span></h2><a className="cyber-button primary big" href="#top">REGISTRATION LINK — TBA <ArrowDownRight size={20}/></a><small>EVENT DATE: TO BE ANNOUNCED · PRIZES: TO BE ANNOUNCED</small></section>
 <footer><div><b>BITHUNT</b><span>// TEAM PARSEC · IIT DHARWAD</span></div><div>CONTACT: 7666804195</div><div>© 2026 PARSEC</div></footer>
 </main>;
}
